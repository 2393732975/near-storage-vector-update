...
实施状态（2026-09-22）：代码与本地协议测试已完成，包括动态 `M`、原子 label
CAS、失败分类、低频探针、对象/PG fan-out 指标和离线一致性检查器。由于实验池已
清理，四数据集三轮集群验收尚待重新导入数据后执行；命令和门槛见
[阶段 0 正确性与观测基线](correctness-observability-baseline.md)。
